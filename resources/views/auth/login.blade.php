<x-layout.public>
    <x-auth.card>
        ...
        <form method="POST" action="{{ route('auth.guest.signin.post') }}">
            @csrf
            ...
            <div class="flex items-center justify-end mt-4">
                @if (Route::has('password.request'))
                    <a class="underline text-sm text-gray-600 hover:text-gray-900" href="{{ route('auth.guest.password.request') }}">
                        {{ __('Forgot your password?') }}
                    </a>
                @endif

                <x-atoms.button class="ml-3">
                    {{ __('Sign In') }}
                </x-atoms.button>
            </div>
        </form>
